...
std::map<std::string, int> noteNameMap = {{"C-2", 5871},
    {"C#-2", 5542},
    {"D-2", 5231},
    {"D#-2", 4937},
    {"E-2", 4660},
...
void Wave::saveWAV(const char *filename) {
    ...
    std::string name(noteNameString);
    
    int numOutputSamples = noteNameMap[name];
    ...
    if(numOutputSamples != 0)
    {
        ...
        float* d = (float*)alloca(sizeof(float) * 8192);
        
        const double ratio =(double)numOutputSamples / (double)WAVE_LEN;
        
        resample(postSamples, WAVE_LEN, d, numOutputSamples, ratio);
        
        sf_write_float(sf, d, numOutputSamples);
